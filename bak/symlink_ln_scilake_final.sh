#!/bin/bash

# Define repo root and target directory
repo_root="/u501/z6dong/Repo" ######################################
target_dir="$repo_root/starmie_internal/data/scilake_final/datalake"

# List of subfolders relative to the repo root
folders=(
    "ModelTables/data/processed/deduped_hugging_csvs"
    "ModelTables/data/processed/deduped_github_csvs"
    "ModelTables/data/processed/tables_output"
    "ModelTables/data/processed/llm_tables"
    "ModelTables/data/processed/deduped_hugging_csvs_transpose"
    "ModelTables/data/processed/deduped_github_csvs_transpose"
    "ModelTables/data/processed/tables_output_transpose"
    "ModelTables/data/processed/llm_tables_transpose"
    "ModelTables/data/processed/deduped_hugging_csvs_str"
    "ModelTables/data/processed/deduped_github_csvs_str"
    "ModelTables/data/processed/tables_output_str"
    "ModelTables/data/processed/llm_tables_str"
    "ModelTables/data/processed/deduped_hugging_csvs_str_transpose"
    "ModelTables/data/processed/deduped_github_csvs_str_transpose"
    "ModelTables/data/processed/tables_output_str_transpose"
    "ModelTables/data/processed/llm_tables_str_transpose"
)

for rel_folder in "${folders[@]}"; do
    folder="$repo_root/$rel_folder"
    echo "Scanning: $folder"
    
    csv_count=$(find "$folder" -maxdepth 1 -iname '*.csv' | wc -l) ########
    echo "Folder: $folder, CSV count: $csv_count"

    find "$folder" -maxdepth 1 -iname '*.csv' | while read -r csv; do ########
        ln -sf "$csv" "$target_dir" ########
    done
done

total_count=$(find "$target_dir" -maxdepth 1 -iname '*.csv' | wc -l) ########
echo "Total CSV count in datalake folder: $total_count"
