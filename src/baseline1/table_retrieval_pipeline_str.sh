#!/bin/bash
# Baseline1: Dense Search Pipeline - STR MODE (string augmentation)
...
filter --base_path data/processed \
        ...
        --output_jsonl data/baseline/valid_tables_str${SUFFIX}.jsonl \
        --model_name all-MiniLM-L6-v2 \
        --device cuda \
        --mode str

python src/baseline1/table_retrieval_pipeline.py \
encode --jsonl data/baseline/valid_tables_str${SUFFIX}.jsonl \
        --model_name all-MiniLM-L6-v2 \
        --batch_size 256 \
        --output_npz data/baseline/valid_tables_str${SUFFIX}_embeddings.npz \
        --device cuda
