...
def filter_keys(data, key_types: List[str]):
    ...
    return {k: v for k, v in data.items() if any(k.endswith(kt + '.csv') for kt in key_types)}

def filter_values(vlist, value_types: List[str]):
    # value_types: list of '', '_s', '_t', etc.
    return [v for v in vlist if any(v.endswith(vt + '.csv') for vt in value_types)]
...
def parse_types(types_str):
    # Support comma-separated or multiple --key_types "" --key_types _s
